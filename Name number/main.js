...
const showRes = document.querySelector('.show-result');
...
function show() {
	nameScore = 0;
	nameInputVal = input.value.toLocaleUpperCase();
	checkCharacter();
	...
	if (nameInputVal.length >= 3) {
		for (let i = 0; i < nameInputVal.length; i++) {
			...
			if (isNaN(parseInt(nameScore))) {
				showRes.innerHTML =
					'<span class="red">Please check your letters</span>';
		...
		showRes.innerHTML =
			'<span class="red">Please enter a name which contains more than 3 characters</span>';
		showRes.style.display = 'block';
	}
}
